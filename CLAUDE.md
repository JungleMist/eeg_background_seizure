...
conda run -n eeg_pipeline python scripts/06_train_xgboost.py [--condition raw|ica|wiener|all] [--force] [--prune-shap]
...
EDF files (TUEP v3.1.0, D:/EEGdata/TUEP/v3.1.0)
  ...
  → features/extraction.py  — extract_epoch_features() → 291-dim vector per epoch (per-channel + asymmetry + connectivity) [experiment/connectivity-only branch: wavelet, complexity, and temporal blocks removed]
...
| `eeg_bg/features/wavelet.py` | `wavelet_features(signal)` → `(27,)` per channel (513 total across 19 channels). PyWavelets `db4`, 6-level DWT (`level 1` ≈ 32–62 Hz γ … `level 6` ≈ 0.5–2 Hz δ). 27 features/channel: detail energy per level (6), modulus-maxima mean per level (6), reconstructed-band signal stats (15). Cut from an original 66/channel (7 subgroups) on 2026-07-01 — entropy, coefficient stats, approximation-coefficient stats, modulus-maxima counts, and scale-energy ratios were dropped as redundant with existing Welch-based features and absent from SHAP top-20s. `WAVELET_NAMES` built at import time. **Not called from `extraction.py` on the `experiment/connectivity-only` branch** — dropped alongside complexity/temporal to isolate the connectivity signal. The module and its own unit tests are untouched. |
| `eeg_bg/features/connectivity.py` | `connectivity_features(epoch, ch_names, sfreq, nperseg)` → `(80,)`. Magnitude-squared coherence + Phase-Locking Value (PLV, via Hilbert transform) for `ALL_PAIRS` = the 8 homotopic pairs in `asymmetry.SYMMETRIC_PAIRS` × 5 bands × 2 metrics. Restricted from all C(19,2) = 171 pairs on 2026-07-01 — inter-hemispheric coherence/PLV is a physiologically distinct signal from power-asymmetry (already captured in `asymmetry.py`) and a documented correlate of lateralized epileptogenic networks. Bandpass+Hilbert phase is computed once per channel per band and reused across pairs for efficiency. |
| `eeg_bg/features/complexity.py` | `complexity_features(epoch, ch_names, m, r_factor)` → `(38,)`. Sample Entropy (embedding dim `m=2`, tolerance `r=0.2×std`, `scipy.spatial.cKDTree` for O(n log n) template matching) + Lempel-Ziv Complexity (binarised at median, normalised by `n/log2(n)`) per channel. **Not called from `extraction.py` on the `experiment/connectivity-only` branch** — it was the single most time-consuming feature block (~49% of per-epoch extraction time on `experiment/connectivity-and-dwt`), so this branch drops it too. The module and its own unit tests are untouched. |
| `eeg_bg/features/temporal_stats.py` | `epoch_temporal_stats(epoch, ch_names, scales)` → `(228,)`. Mean/variance/skewness/kurtosis computed per non-overlapping window then averaged across windows, at 3 scales (125/375/750 samples = 1 s/3 s/6 s at 125 Hz) per channel. **Not called from `extraction.py` on the `experiment/connectivity-only` branch** — the second most time-consuming block (~29% of per-epoch extraction time); dropped for the same reason as `complexity.py` above. The module and its own unit tests are untouched. |
| `eeg_bg/features/extraction.py` | `extract_epoch_features(epoch, ch_names, sfreq)` → `(291,)` vector on the `experiment/connectivity-only` branch (`(1070,)` on `master`, which additionally concatenates the wavelet, complexity, and temporal blocks); `build_dataset(cache_root, condition, split, ...)` → `(X, y, subject_ids)`. Feature cache in `cache/features/{condition}_{split}.npz`. |
...
| `eeg_bg/features/extraction.py` (constants) | `FEATURE_NAMES` — public list of 291 strings on `experiment/connectivity-only` (1070 on `master`) built at import time; positions 0–210 are positionally stable (see below) since SHAP `.npy` arrays are indexed by position. `_CONDITION_TO_SUBDIR` maps `"wiener"→"wiener_frequency"` etc. |
...
**This section describes the `experiment/connectivity-only` branch.** `extract_epoch_features` produces a **291-dim vector**, built by concatenating three blocks in this order:
...
| `[211:291]` | 80 | Connectivity | 8 homotopic pairs (`asymmetry.SYMMETRIC_PAIRS`) × 5 bands × 2 metrics (coherence, PLV) |

On `master`, three further blocks trail this vector to make a 1070-dim vector: `[211:724]` Wavelet DWT (513 dims), `[724:804]` Connectivity (same 80 dims, just at a different offset), `[804:842]` Complexity (38 dims, SampEn+LZC per channel), and `[842:1070]` Multi-scale temporal stats (228 dims). `experiment/connectivity-only` branches directly off `master` and removes wavelet, complexity, and temporal in one step — profiling on the sibling `experiment/connectivity-and-dwt` branch (which drops only complexity/temporal) showed those two were the most expensive blocks (~49% and ~29% of per-epoch time); wavelet is the next-most-expensive remaining block, so this branch isolates the connectivity signal (per-channel + asymmetry + connectivity only) for the fastest feature extraction of the three variants. The `eeg_bg/features/{wavelet,complexity,temporal_stats}.py` modules and their unit tests still exist on this branch; they're simply not called from `extraction.py`.

**Positions 0–210 are positionally stable** — this is the original 211-dim vector and predates the wavelet/connectivity blocks; reordering `standard_19` or `SYMMETRIC_PAIRS` invalidates saved SHAP `.npy` arrays (indexed by position, not name). The blocks after position 210 were added later (see `docs/superpowers/plans/2026-06-10-feature-engineering-expansion.md`) and were reduced on 2026-07-01 to curb overfitting risk against the ~124-subject training set (wavelet 66→27 features/channel, connectivity 171→8 pairs); SHAP artifacts and feature caches from before that date are not comparable to current runs. `experiment/connectivity-only`'s 291-dim vectors are a further, disjoint reduction from `master`'s 1070-dim vectors — the two are not interchangeable and feature caches / SHAP `.npy` arrays are not shared between branches.

**`ml.features` in `configs/default.yaml` is currently dead config** — it documents intended hyperparameters for the newer blocks (`wavelet.wavelet`/`wavelet.level`, `connectivity.nperseg`, `complexity.m`/`complexity.r_factor`, `temporal.scales`), but nothing in the codebase reads `cfg["ml"]["features"]`. `extraction.py` calls `connectivity_features()` (and, on `master` only, `wavelet_features()`/`complexity_features()`/`epoch_temporal_stats()`) with no extra arguments, so they silently fall back to the hardcoded defaults in their own modules (`db4`/level 6, `m=2`/`r_factor=0.2`, `scales=[125,375,750]`) — which happen to match the YAML values today, but editing the YAML has no effect. The only feature-extraction params actually threaded through from config are `preprocessing.target_sfreq`, `wiener.nperseg`, and `wiener.freq_band` (passed into `build_dataset`/`extract_epoch_features` from `scripts/06_train_xgboost.py`), which drive the per-channel block and the connectivity block's `nperseg`/`sfreq`. To actually change wavelet/complexity/temporal behavior, edit the defaults in `eeg_bg/features/{wavelet,complexity,temporal_stats}.py` directly, or wire `cfg["ml"]["features"]` through `extract_epoch_features`'s call sites.
...
```
The `wiener` condition in `build_dataset` / `--condition` maps to `cache/wiener_frequency/` (not `cache/wiener/`). `eeg_bg/ml/cnn_dataset.py`'s `EEGEpochDataset` reads the same `cache/{epochs,wiener_frequency,ica}/` trees directly (no separate CNN cache) — it loads raw `(19, 1000)` tensors rather than the 291-dim (`experiment/connectivity-only`) / 1070-dim (`master`) feature vectors.
...
│   │   ├── shap_values_test.npy      — (n_test_epochs, 291) raw SHAP values (experiment/connectivity-only branch; 1070 on master)
...
│   ├── pruning_report.json           — features with mean |SHAP| < ml.shap.pruning_threshold (from --prune-shap)
...
- **`tests/test_ml/conftest.py`**: `tiny_xgb_model` (10-feature, 5-estimator, session-scoped), `full_feature_xgb_model` (`len(FEATURE_NAMES)`-feature, session-scoped — 291 on `experiment/connectivity-only`, 1070 on `master`).
...
- **`FEATURE_NAMES` (291 entries on `experiment/connectivity-only`, 1070 on `master`) must stay positionally stable**: SHAP `.npy` arrays are indexed by position against `FEATURE_NAMES`. Any reordering of channels in `configs/default.yaml` `standard_19`, of pairs in `asymmetry.SYMMETRIC_PAIRS`, or of the block order in `eeg_bg/features/extraction.py` invalidates saved SHAP arrays. Script 06 guards against a stale feature-dim mismatch and raises `ValueError` pointing at `--force`. Note: `connectivity.ALL_PAIRS` now derives from `asymmetry.SYMMETRIC_PAIRS`, so reordering `SYMMETRIC_PAIRS` invalidates both the asymmetry block *and* the connectivity block. Feature caches (`cache/features/*.npz`) and SHAP artifacts are not interchangeable between `experiment/connectivity-only` and `master` — the dimension mismatch will trip the `--force` guard if you switch branches without re-extracting.
...
- **`docs/Developer_qa.md`** — Detailed Q&A: epoch validity criteria (4 criteria with decision flow), `.npz` cache file schemas (exact keys/shapes/dtypes per cache family), feature extraction internals (band definitions, Hjorth formulae, spectral entropy formula, per-condition signal mapping). Predates the wavelet/connectivity/complexity/temporal/CNN additions — cross-check numeric claims (e.g. it describes the older 211-dim feature layout and stale label-dtype info) against the code before relying on it.
