...
EDF files (TUEP v3.1.0, D:/EEGdata/TUEP/v3.1.0)
  ...
  → features/extraction.py  — extract_epoch_features() → 804-dim vector per epoch (per-channel + asymmetry + wavelet + connectivity) [lite branch: complexity + temporal blocks removed]
...
| `eeg_bg/features/extraction.py` (constants) | `FEATURE_NAMES` — public list of 804 strings on `lite` (1070 on `master`) built at import time; positions 0–210 are positionally stable (see below) since SHAP `.npy` arrays are indexed by position. `_CONDITION_TO_SUBDIR` maps `"wiener"→"wiener_frequency"` etc. |
...
**This section describes the `lite` branch.** `extract_epoch_features` produces an **804-dim vector**, built by concatenating four blocks in this order:
...
│   │   ├── shap_values_test.npy      — (n_test_epochs, 804) raw SHAP values (lite branch; 1070 on master)
...
- **`tests/test_ml/conftest.py`**: `tiny_xgb_model` (10-feature, 5-estimator, session-scoped), `full_feature_xgb_model` (`len(FEATURE_NAMES)`-feature, session-scoped — 804 on `lite`, 1070 on `master`).
...
- **`FEATURE_NAMES` (804 entries on `lite`, 1070 on `master`) must stay positionally stable**: SHAP `.npy` arrays are indexed by position against `FEATURE_NAMES`. Any reordering of channels in `configs/default.yaml` `standard_19`, of pairs in `asymmetry.SYMMETRIC_PAIRS`, or of the block order in `eeg_bg/features/extraction.py` invalidates saved SHAP arrays. Script 06 guards against a stale feature-dim mismatch and raises `ValueError` pointing at `--force`. Note: `connectivity.ALL_PAIRS` now derives from `asymmetry.SYMMETRIC_PAIRS`, so reordering `SYMMETRIC_PAIRS` invalidates both the asymmetry block *and* the connectivity block. Feature caches (`cache/features/*.npz`) and SHAP artifacts are not interchangeable between `lite` and `master` — the dimension mismatch will trip the `--force` guard if you switch branches without re-extracting.
