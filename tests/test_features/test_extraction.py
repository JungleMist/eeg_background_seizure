...
def test_build_dataset_wiener_zerophase_condition(tmp_path):
    """wiener_zerophase resolves via _CONDITION_TO_SUBDIR/_CONDITION_TO_KEY,
    mirroring how the ica/wiener conditions read their 'specific' key."""
    rng = np.random.default_rng(5)
    ep_arr = rng.standard_normal((3, 19, 1000)).astype(np.float64)
    path = tmp_path / "wiener_zerophase" / "s001" / "ep.npz"
    path.parent.mkdir(parents=True, exist_ok=True)
    np.savez(
        path,
        specific=ep_arr,
    ...
    X, y, sids = build_dataset(tmp_path, "wiener_zerophase", "train",
                                sfreq=125.0, nperseg=250,
                                freq_band=(0.5, 40.0))
    assert X.shape == (3, 211)
    assert y.shape == (3,)
...
def test_build_dataset_stale_cache_raises(tmp_path):
    ...
    np.savez(feat_cache,
             X=np.zeros((2, 1070), dtype=np.float64),
