...
def test_feature_names_per_channel():
    """Each channel should have exactly 9 + 27 + 2 + 12 = 50 feature names."""
    for ch in _STANDARD_19:
        ch_feats = [n for n in FEATURE_NAMES if n.startswith(f"{ch}_")]
        assert len(ch_feats) == 50, f"Expected 50 features for {ch}, got {len(ch_feats)}"
...
def test_extract_epoch_features_missing_channel(sfreq):
    ...
    other_indices = [i for i in range(1070) if i not in fp1_indices]
