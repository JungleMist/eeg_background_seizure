...
def test_feature_names_per_channel():
    """Each channel should have exactly 9 + 27 = 36 feature names (lite: no complexity/temporal)."""
    for ch in _STANDARD_19:
        ch_feats = [n for n in FEATURE_NAMES if n.startswith(f"{ch}_")]
        assert len(ch_feats) == 36, f"Expected 36 features for {ch}, got {len(ch_feats)}"
...
def test_extract_epoch_features_missing_channel(sfreq):
    ...
    other_indices = [i for i in range(804) if i not in fp1_indices]
