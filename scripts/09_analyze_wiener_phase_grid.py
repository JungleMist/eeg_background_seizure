...
def _load_predictions(results_root: Path, profile: str, condition: str) -> pd.DataFrame:
    path = results_root / profile / condition / "test_predictions.csv"
    if not path.exists():
    ...
    return df[["subject_id", "pred_proba", "true_label"]].rename(
        columns={"true_label": "true_label"}
    )


def analyze(results_parent: Path, output_dir: Path, repeats: int, seed: int) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    rows = []
    paired = []
    for idx, mode, phase, coh in MATRIX:
        exp_root = results_parent / f"exp{idx}"
        cfg_path = Path("configs") / f"exp_wiener_phase_{idx}.yaml"
    ...
    for idx, mode, _phase, _coh in MATRIX:
        ...
        if left_path.exists() and right_path.exists():
            profile_rows.append({
                ...
                **_bootstrap_delta(pd.read_csv(left_path), pd.read_csv(right_path), repeats, seed),
            })
    if profile_rows:
...
def main() -> None:
    ...
    analyze(root, out, args.bootstrap_repeats, args.bootstrap_seed)
